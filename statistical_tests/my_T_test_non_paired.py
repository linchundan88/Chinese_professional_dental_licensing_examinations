    ...
    for model1 in list_model_name1:
        ...
        for unit_num1 in list_unit1:
            ...
            for i, unit in enumerate(list_units):
                if unit == unit_num1:
                    pred1 = parse_result(list_prediction_answer1[i])
                    if pred1 in ['A', 'B', 'C', 'D', 'E']:
                        list_predictions_unit1.append(1 if pred1 == list_correct_answers1[i] else 0)
                elif unit == unit_num2:
                    pred2 = parse_result(list_prediction_answer1[i])
                    if pred2 in ['A', 'B', 'C', 'D', 'E']:
                        list_predictions_unit2.append(1 if pred2 == list_correct_answers1[i] else 0)
            ...
            # 执行独立样本单边 T 检验 (unpaired one-tailed t-test)
            # 检验 unit1 的得分是否显著高于 unit2
            ...
            # alternative='greater' 表示备择假设是 unit1 > unit2
